 ...
 * 
 * InteractionLogger.$inject = [ 'eventBus' ]; // minification save // extension
 * module var extensionModule = { __init__: [ 'interactionLogger' ],
 * interactionLogger: [ 'type', InteractionLogger ] }; // extend the viewer var
 * bpmnViewer = new Viewer({ additionalModules: [ extensionModule ] });
...
function PathMap() {
  ...
  /**
	 ...
	 * <li>Position is used to set the starting coordinate of the path. M is
	 * computed:
	 * <ul>
 ...
 * canvas.addShape({ x: 10, y: 10 }); }); // 'shape ... was added to the
 * diagram' logged to console
 * 
 * @param {Object}
 *            options
 * @param {Array
 ...
 * visible area of the diagram to (100|100) -> (600|100) // and and scales it
 * according to the diagram width
 * 
 ...
 * access event type event.type; // 'foo' // stop propagation to other listeners
 * event.stopPropagation(); // prevent event default event.preventDefault(); }); //
 * listen for event with custom payload eventBus.on('bar', function(event,
 * payload) { console.log(payload); }); // listen for event returning value
 * eventBus.on('foobar', function(event) { // stop event propagation + prevent
 * default return false; // stop event propagation + return custom result return {
 ...
 * 
 * ```javascript // false indicates that the default action // was prevented by
 * listeners if (eventBus.fire('foo') === false) { console.log('default has been
 * prevented!'); };
 *  // custom args + return value listener eventBus.on('sum', function(event, a,
 * b) { return a + b; }); // you can pass custom arguments + retrieve result
 * values. var sum = eventBus.fire('sum', 1, 2); console.log(sum); // 3 ```
 */
function EventBus() {
  this._listeners = {};
 ...
 * Fires a named event.
 * 
 * @example // fire event by name events.fire('foo'); // fire event object with
 *          nested type var event = { type: 'foo' }; events.fire(event); // fire
 *          event with explicit type var event = { x: 10, y: 20 };
 *          events.fire('element.moved', event); // pass additional arguments to
 *          the event events.on('foo', function(event, bar) { alert(bar); });
 * 
 ...
 * 
 * It emits the following events: * element.hover * element.out * element.click *
 * element.dblclick * element.mousedown
 * 
 ...
 * style="width: 10px; background: fuchsia; color: white;">0</div>' }); // or
...
(function (glob) {
	 ...
	 * separated - scope (object) context for the event handlers - varargs (...)
	 * the rest of arguments will be sent to event handlers = (object) array of
	 * returned values from the listeners. Array has two methods
	 * `.firstDefined()` and `.lastDefined()` to get first or last not
	 * `undefined` value. \
	 */
	 ...
	 * triggered by the given `name`. > Arguments - name (string) name of the
	 * event, dot (`.`) or slash (`/`) separated = (array) array of event
	 * handlers \
	 */
 ...
 * any object or primitive - type (string) name of the type, e.g., `string`,
 * `function`, `number`, etc. = (boolean) `true` if given value is of given type \
 */
Snap.is = is;
/*
 ...
 * shortened HTML color: (<code>#000</code>, <code>#fc0</code>, etc.)</li> #
 * <li>#•••••• — full length HTML color: (<code>#000000</code>, <code>#bd2300</code>)</li> #
 * <li>rgb(•••, •••, •••) — red, green and blue channels values: (<code>rgb(200,&nbsp;100,&nbsp;0)</code>)</li> #
 * <li>rgba(•••, •••, •••, •••) — also with opacity</li> # <li>rgb(•••%,
 * •••%, •••%) — same as above, but in %: (<code>rgb(100%,&nbsp;175%,&nbsp;0%)</code>)</li> #
 * <li>rgba(•••%, •••%, •••%, •••%) — also with opacity</li> # <li>hsb(•••,
 * •••, •••) — hue, saturation and brightness values: (<code>hsb(0.5,&nbsp;0.25,&nbsp;1)</code>)</li> #
 * <li>hsba(•••, •••, •••, •••) — also with opacity</li> # <li>hsb(•••%,
 * •••%, •••%) — same as above, but in %</li> # <li>hsba(•••%, •••%, •••%,
 * •••%) — also with opacity</li> # <li>hsl(•••, •••, •••) — hue, saturation
 * and luminosity values: (<code>hsb(0.5,&nbsp;0.25,&nbsp;0.5)</code>)</li> #
 * <li>hsla(•••, •••, •••, •••) — also with opacity</li> # <li>hsl(•••%,
 * •••%, •••%) — same as above, but in %</li> # <li>hsla(•••%, •••%, •••%,
 * •••%) — also with opacity</li> # </ul> Note that `%` can be used any time:
 ...
 * HTML/CSS format: #••••••, o error (boolean) true if string can't be parsed o } \
 */
 ...
 * (number) blue, o hex (string) color in HTML/CSS format: #••••••, o error
 * (boolean) `true` if string can't be parsed, o h (number) hue, o s (number)
 * saturation, o v (number) value (brightness), o l (number) lightness o } \
 */
 ...
 * (number) blue, o hex (string) color in HTML/CSS format: #•••••• o } \
 */
 ...
 * blue, o hex (string) color in HTML/CSS format: #•••••• o } \
 */
 ...
 * elements or strings * - varargs (…) SVG string = (Fragment) the @Fragment \
 */
...
// they also be assigned default values?
	 ...
	 * (number) 0..1 — status of the animation: 0 — just started, 1 — just
	 * finished, o status (function) gets or sets the status of the animation, o
	 * stop (function) stops the animation o } \
	 */
...
Snap.plugin(function (Snap, Element, Paper, glob, Fragment) {
	 ...
	 * them in relative terms from the most recently declared coordinates. # <p>Here
	 * is short list of commands available, for more details see <a
	 ...
	 * <tr><td>A</td><td>elliptical arc</td><td>(rx ry x-axis-rotation
	 * large-arc-flag sweep-flag x y)+</td></tr> # <tr><td>R</td><td><a
	 ...
	 * three commands: `M10,10R…z`. In this case the path connects back to its
	 * starting point. > Usage | var c = paper.path("M10 10L90 90"); | // draw a
	 * diagonal line: | // move to 10,10, line to 90,90 \
	 */
    ...
    /*
	 * \ Paper.g [ method ] * Creates a group element * - varargs (…) #optional
	 * elements to nest within the group = (object) the `g` element * > Usage |
	 * var c1 = paper.circle(), | c2 = paper.rect(), | g = paper.g(c2, c1); //
	 * note that the order of elements is different or | var c1 =
	 ...
	 * it’s a pattern. - x (number) @optional X of the element - y (number)
	 * @optional Y of the element - width (number) @optional width of the
	 * element - height (number) @optional height of the element - vbx (number)
...
Snap.plugin(function (Snap, Element, Paper, glob) {
	 ...
	 * coordinates on the given cubic beziér curve at the given t - p1x (number)
	 * x of the first point of the curve - p1y (number) y of the first point of
	 * the curve - c1x (number) x of the first anchor of the curve - c1y
	 * (number) y of the first anchor of the curve - c2x (number) x of the
	 ...
	 * box of a given cubic beziér curve - p1x (number) x of the first point of
	 * the curve - p1y (number) y of the first point of the curve - c1x (number)
	 * x of the first anchor of the curve - c1y (number) y of the first anchor
	 * of the curve - c2x (number) x of the second anchor of the curve - c2y
	 * (number) y of the second anchor of the curve - p2x (number) x of the
	 ...
	 * eight coordinates representing beziér curve for the segment of path1, o
	 * bez2: (array) eight coordinates representing beziér curve for the segment
	 * of path2 o } o ] \
	 */
	 ...
	 * 
	 * Note: fill mode doesn’t affect the result of this method. - path (string)
	 * path string - x (number) x of the point - y (number) y of the point =
...
snapsvg.plugin(function (Snap, Element, Paper, global) {
  ...
  /*
	 ...
	 * @example
	 * 
	 * e.translate(10, 20); // sets transform matrix to translate(10, 20)
	 * 
 ...
 * 'active': false } ]; // using the `_.matches` callback shorthand
 * _.every(users, { 'user': 'barney', 'active': false }); // => false // using
 * the `_.matchesProperty` callback shorthand _.every(users, 'active', false); // =>
 * true // using the `_.property` callback shorthand _.every(users, 'active'); // =>
 * false
 */
 ...
 * 'fred', 'age': 40, 'active': false } ]; // using the `_.matches` callback
 * shorthand _.pluck(_.filter(users, { 'age': 36, 'active': true }), 'user'); // =>
 * ['barney'] // using the `_.matchesProperty` callback shorthand
 * _.pluck(_.filter(users, 'active', false), 'user'); // => ['fred'] // using
 * the `_.property` callback shorthand _.pluck(_.filter(users, 'active'),
 * 'user'); // => ['barney']
 */
 ...
 * 'barney' // using the `_.matches` callback shorthand _.result(_.find(users, {
 * 'age': 1, 'active': true }), 'user'); // => 'pebbles' // using the
 * `_.matchesProperty` callback shorthand _.result(_.find(users, 'active',
 * false), 'user'); // => 'fred' // using the `_.property` callback shorthand
 * _.result(_.find(users, 'active'), 'user'); // => 'barney'
 */
 ...
 * '4': [4.2], '6': [6.1, 6.4] } // using the `_.property` callback shorthand
 * _.groupBy(['one', 'two', 'three'], 'length'); // => { '3': ['one', 'two'],
 * '5': ['three'] }
 */
 ...
 * guaranteed)
 * 
 * var users = [ { 'user': 'barney' }, { 'user': 'fred' } ]; // using the
 * `_.property` callback shorthand _.map(users, 'user'); // => ['barney',
 * 'fred']
 */
 ...
 * 'active': false } ]; // using the `_.matches` callback shorthand
 * _.some(users, { 'user': 'barney', 'active': false }); // => false // using
 * the `_.matchesProperty` callback shorthand _.some(users, 'active', false); // =>
 * true // using the `_.property` callback shorthand _.some(users, 'active'); // =>
 * true
 */
 ...
 *          subsequent calls jQuery('#postbox').on('click', _.debounce(sendMail,
 *          300, { 'leading': true, 'trailing': false })); // ensure `batchLog`
 *          is invoked once after 1 second of debounced calls var source = new
 *          EventSource('/stream'); jQuery(source).on('message',
 *          _.debounce(batchLog, 250, { 'maxWait': 1000 })); // cancel a
 *          debounced call var todoChanges = _.debounce(batchLog, 1000);
 ...
 * ...before 1 second has passed `models.todo` is deleted // which cancels the
 * debounced `todoChanges` call delete models.todo;
 */
 ...
 * 'user': 'fred', 'age': 40 } // using a customizer callback var defaults =
 * _.partialRight(_.assign, function(value, other) { return _.isUndefined(value) ?
 * other : value; });
 * 
 ...
 * 'user': 'fred', 'age': 40 }] } // using a customizer callback var object = {
